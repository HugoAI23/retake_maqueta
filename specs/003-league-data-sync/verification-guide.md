...
| **Abierto** · conexiones con la base | Contar las conexiones del backend con PostgreSQL (`select count(*) from pg_stat_activity where datname = 'retake'`) | Tras las pruebas del navegador, el backend de desarrollo llegó a 95 conexiones y agotó `max_connections`. No se ha podido reproducir (`curl`, proxy y navegador liberan las conexiones del canal al cerrarse). El 2026-09-24, con `retake sync` en modo real: 3 de 100. Vigilar; si se repite, limitar el número de canales o compartir una sola escucha `LISTEN` por proceso. |
