...
| Fase | Qué comprobar |
|---|---|
| F4b | `uv run --directory backend retake import-wiki-csv` → `Resultado: success`, 14 campeonatos, 284 clasificaciones, 169 franquicias (168 de los Champs y Cloud9 NY del roster 2026, C-28) y 515 jugadores; `http://localhost:8000/api/championships` muestra 2013 a 2026. |
...
| RF-4 | Terminal | `uv run --directory backend retake import-wiki-csv` · pt `tests/unit/sources/test_wiki_csv.py -k rosters` | Importa el historial completo, el nombre de cada equipo de los rosters por temporada (C-28; así entra «Cloud9 NY», que aún no ha jugado un Champs) y los datos personales de sus jugadores. |
| RF-4a | Prueba | pt `tests/unit/sources/test_wiki_csv.py -k datos_personales` | De `players_birthday.csv` solo entran los jugadores del historial o de un roster. Van por gamertag exacto; sin distinguir mayúsculas solo si ningún otro jugador se escribe igual salvo las mayúsculas (I-46: LuCkY no recibe los datos de Lucky). |
...
| **Resuelto (I-47)** · conexiones con la base | pt `tests/integration/api/test_003_pool.py tests/integration/api/test_003_stream.py` · con el backend en marcha, `npx playwright test` y contar las conexiones (`select count(*) from pg_stat_activity where datname = 'retake'`) | Las peticiones no se bloquean con el pool y los hilos llenos, y las escuchas `LISTEN` del canal se cierran aunque falle la primera consulta o la página se vaya con un envío a medias. Con la batería completa: máximo 22 conexiones y ninguna escucha al terminar (antes, más de 100 y 83 escuchas para siempre). |
| **Resuelto (I-45)** · nombres anteriores en el historial | pt `tests/integration/ingest/test_003_curation.py -k "nombre or reimportar or sigue_con_el_actual"` · `http://localhost:8000/api/championships` | Cada Champs lleva el nombre de la franquicia en su final: en 2020 el campeón es `DAL Empire`, con el mismo `franchiseId` que OpTic Texas de 2022 a 2026; NY Subliners de 2020 a 2024 dentro de Cloud9 New York. El resultado no depende de si la unión se aplica antes o después de importar. |
| **Resuelto (I-46)** · datos personales de otro gamertag | pt `tests/unit/sources/test_wiki_csv.py -k mayusculas` | `wiki:LuCkY`, `wiki:Super`, `wiki:Aqua` y `wiki:Methodz` ya no reciben los datos de Lucky, SupeR, AquA y MethodZ; los 11 datos guardados por error se borraron a mano con el permiso de Hugo. |
...
| Curación | **Hecha el 2026-09-24:** 13 países, 64 jugadores unidos, 23 uniones de franquicias (12 de nombres anteriores, I-45) y 62 roles de 2026 confirmados por Hugo. `retake list-retained` deja 597 retenidos (franquicias y jugadores antiguos del historial, C-14). Se repite tras cada carga que traiga jugadores o franquicias nuevos. |
