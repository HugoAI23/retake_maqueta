...
| I-43 | T-087 | **`/admin` con el servidor caído** (visto por Hugo al verificar RF-115). Si no se podía comprobar la sesión, la página mostraba la pantalla de acceso, como si no hubiera sesión, y el formulario daba el mismo aviso genérico ante cualquier fallo: parecía que la contraseña estaba mal. **Ahora:** solo un 401 lleva a la pantalla de acceso; cualquier otro fallo al comprobar la sesión muestra el error de bloque de la 001 con "Reintentar" (RF-115), que vuelve a comprobarla. En el formulario, sin respuesta o con un 5xx el aviso es "No se ha podido conectar con el servidor" (`admin.login.unreachable`); las credenciales incorrectas siguen con su único mensaje (RF-126). Dos pruebas nuevas en `AdminPage.test.jsx`. | Que un servidor caído no se confunda con una contraseña incorrecta. | RF-115, RF-124, RF-126 |
| I-44 | T-087 | **Datos en vivo sin caché del navegador** (visto por Hugo al verificar RF-79 en Safari con la fuente simulada). Con la pestaña a la vista y sin recargar, el bloque en vivo seguía con el marcador anterior; al recargar salía el correcto; en Chromium se actualizaba bien. Safari devolvía una copia guardada de `/api/matches`, que no llevaba ninguna cabecera de caché. **Ahora:** un middleware ASGI (`NoStoreApiMiddleware`, puro para no interferir con el canal de eventos) añade `Cache-Control: no-store` a toda respuesta de `/api/` que no fije la suya (los logos siguen inmutables y el canal con `no-cache`), y los clientes del frontend piden con `cache: 'no-store'`. **Límite del modo simulado anotado:** el aviso de datos sin actualizar no aparece en una prueba manual simulada, porque el proceso de obtención anota con la hora del escenario (I-32) y la API compara con la hora real; RF-89 a RF-93 quedan cubiertos por sus pruebas automáticas. **Herramienta de verificación:** `vite.config.js` acepta `RETAKE_API_TARGET` para apuntar la web de desarrollo a otro backend (el simulado sobre la base `retake_sim`, sin tocar la real). | Que un bloque en vivo muestre siempre los datos de ahora. | RF-79, RF-82 |
| I-45 | T-086 | **Nombres anteriores de las franquicias en el historial** (curación real, 2026-09-24; cambio C-28, aprobado por Hugo el mismo día). Al unir `wiki:DAL_Empire` a OpTic Texas, el nombre DAL Empire desaparecía y los Champs 2020 y 2021 salían como OpTic TEX: la Wiki no publica desde cuándo vale un nombre, todas sus identidades tenían la hora de la importación y, en la misma franquicia, se pisaban (RF-13 y RF-74 de la 002). **Conector de la Wiki:** cada identidad publica su fecha de vigencia: el día siguiente (00:00 UTC) a la final del año anterior a la primera vez que aparece ese nombre, en un Champs o en un roster de temporada; sin esa final legible, el 1 de enero de ese año (decisión de Hugo: los cambios de nombre caen fuera de temporada, así los partidos de 2026 llevan el nombre nuevo). Por C-28 también lee el equipo de `cdl_<año>_rosters.csv` (columna `Team`, ahora obligatoria): es el único nombre que la Wiki da de Cloud9 New York, que aún no ha jugado un Champs; sin él, NY Subliners se tomaba por su nombre vigente. **Identidades** (`resolve_identity`): (1) la fila que deja una fuente al sumarse a la identidad vigente, si se queda sin referencias, se retira y sus clasificaciones pasan a la combinada; si ninguna fuente publica fecha, la combinada toma la más antigua de las retiradas (lo que ya pedía I-37); el nombre anterior de una misma fuente sigue siendo historial (RF-73). (2) Las versiones anteriores de la misma fuente que estaban en la fila vigente se vuelven a resolver como anteriores. (3) Una identidad anterior guardada sin fecha toma la que publica la fuente si no salta por encima de otra. Con (2) y (3) el resultado no depende del orden: igual si la unión se aplica tras importar que si ya está en la curación al importar (instalación nueva o importación tras cada Champs). **Curación:** 12 uniones de franquicias confirmadas por Hugo fila a fila (RF-114 de la 002); Chicago → M80 Boston queda fuera por dudosa. **Comprobado** en una copia de la base de desarrollo (`retake_curtest`): cada Champs con su nombre de entonces, ninguna identidad sin referencias, una segunda importación no cambia nada y 11 franquicias retenidas menos. | Que el historial de Champs cuelgue de la franquicia actual sin perder el nombre de cada época. | RF-4 (C-28); RF-12, RF-13, RF-73, RF-74, RF-114 de la 002; RF-61 a RF-64 |
| I-46 | T-086 | **Datos personales de otro jugador con el mismo gamertag salvo mayúsculas** (visto por Hugo al decidir si `wiki:LuCkY` y `wiki:Super` eran `bp:93` y `bp:135`; no lo son: otras personas, de otros equipos y épocas). El conector de la Wiki buscaba los datos personales (archivo de cumpleaños y rosters) por gamertag sin distinguir mayúsculas, así que LuCkY (Champs 2013) recibía los de Lucky (roster 2026), y lo mismo Super, Aqua y Methodz. **Ahora:** por gamertag exacto; sin distinguir mayúsculas solo si ningún otro jugador de los archivos se escribe igual salvo las mayúsculas (los choques de los archivos reales: AquA/Aqua, EndurAAA/endurAAA, LuCkY/Lucky, MethodZ/Methodz, SupeR/Super). Si dos formas son la misma persona, se unen con `player_merges`. **Base de desarrollo:** reimportar no borra un dato que la fuente deja de publicar (las fuentes publican por partes), así que, con el permiso de Hugo, se borraron a mano las 11 observaciones de nombre real, país y fecha de nacimiento de `wiki:LuCkY`, `wiki:Super`, `wiki:Aqua` y `wiki:Methodz`, y se reimportó la Wiki. **Curación:** `wiki:LuCkY` y `wiki:Super` quedan sin unir, con el motivo anotado. | Que ningún jugador muestre los datos personales de otra persona. | RF-4, RF-4a; RF-54 |
| I-47 | T-087 | **Conexiones con PostgreSQL agotadas bajo carga** (la fila abierta de la guía; reproducido el 2026-09-24 con la batería de extremo a extremo contra un backend ya en marcha: más de 100 conexiones, `too many clients` y 83 escuchas `LISTEN` que no se cerraban nunca). Dos causas: (1) **Bloqueo entre hilos y pool:** las rutas síncronas validan su respuesta y cierran su sesión en un hilo de la reserva de AnyIO (40); bajo carga, todos los hilos esperaban una conexión del pool (15) mientras las peticiones que la tenían esperaban un hilo, hasta caducar a los 30 s. **Ahora** `get_session` es asíncrona y no deja entrar a la vez más peticiones con sesión que conexiones tiene el pool (`_db_slots`, un semáforo por motor y bucle de eventos); las demás esperan en el bucle sin ocupar ningún hilo. (2) **Escuchas huérfanas del canal de eventos:** la primera consulta de frescura se hacía fuera del bloque que cierra la escucha, así que un fallo o una cancelación allí la dejaban abierta, y el navegador reconecta cada 5 s; además, si la página se iba con un envío a medias, Starlette cancelaba el envío y el generador se quedaba suspendido sin llegar a su `finally`. **Ahora** la escucha arranca dentro de ese bloque y `EventStreamResponse` cierra siempre el generador, protegido de la cancelación. **Comprobado:** con la misma batería, máximo 22 conexiones y 4 escuchas, ninguna al terminar, ningún error en el registro, y el backend se cierra con un solo Ctrl+C (antes se quedaba esperando a los canales). La prueba de extremo a extremo de RF-68 de la 001, que fallaba en la batería completa al recargar, pasa: era el pool atascado. **Visto de paso:** en desarrollo (HTTP/1.1), con 6 pestañas de Retake abiertas en el mismo navegador, la 7.ª no carga, porque cada una ocupa una conexión con su canal; es lo que se sospechaba para RF-79 en Safari; con HTTP/2 no pasa. | Que ninguna carga agote la base de datos ni deje conexiones abiertas. | RF-79 a RF-81, RF-89; plan §3.3 y D-6 |
