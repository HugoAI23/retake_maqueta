...
|---|---|---|
| ~~La spec 002 sigue en revisión~~ | RF-58 (año de temporada) y RF-70 (lista de etiquetas) dependen de ella. | Año provisional (P-6). ~~Las etiquetas se toman de la versión actual de la 002 y se revisan cuando se apruebe.~~ **Cerrado el 2026-09-22:** la 002 está aprobada y las etiquetas se ajustaron (I-12). El año sigue siendo provisional hasta la spec 003 (nota de RF-3 de la 002). |
...
| I-12 | (2026-09-22, tras aprobar la spec 002) En los diccionarios `es` y `en`, la fase `final` se sustituye por `grandFinal` ("gran final" / "grand final") y se añaden `toBeDecided`, `winnerOf`, `loserOf`, `notPlayed`, `corrected`, `freeAgent` y `standingsUnavailable`. `winnerOf` y `loserOf` reciben el partido de origen como parámetro `{{match}}`. Las siglas `DQ`, `SMG` y `AR` no entran en los diccionarios. La prueba de diccionarios comprueba las cinco fases exactas y que las siglas no se traducen. | La lista de §5.2 de la spec 002 aprobada (decisiones C-9, K-4 y K-5 de la 002). | RF-70 |
| I-13 | (2026-09-22, detectado en la fase F7 de la spec 002) `PageTransition` animaba también la primera carga en desarrollo: `StrictMode` monta los efectos dos veces y la marca de "primera vez" sobrevivía al doble montaje. Ahora el componente recuerda la última dirección y solo anima cuando cambia. Nueva prueba `src/motion/PageTransition.test.jsx` (con `StrictMode`: la primera carga no se anima, una navegación anima una vez y con reducir movimiento no se anima). | Durante los 200 ms del fundido, la auditoría axe del proyecto `dev` medía poco contraste y fallaba de forma intermitente. En producción no ocurría. El comportamiento aprobado no cambia. | RF-73, RF-85, RF-88 |
